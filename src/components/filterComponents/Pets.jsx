...
const Pets = () => {
  ...
  // ✅ Filter products that contain the "pet" tag
  const petProducts = products.filter((product) => {
    if (Array.isArray(product.tags)) {
      return product.tags.includes("pet");
    } else if (typeof product.tags === "string") {
      return product.tags.split(",").map(tag => tag.trim()).includes("pet");
    }
    return false;
  });
  ...
  return (
    <div className="container mx-auto px-6 lg:px-16 pt-16">
      <h2 className="font-semibold text-3xl text-center pb-6 text-primary">Pet Products</h2>
